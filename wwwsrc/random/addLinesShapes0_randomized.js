...
item.genRandomPointOnSegment = function (seg) {
  ...
  let vec = end1.difference(end0);
  let rn = Math.random();
  return {fraction:rn,value:end0.plus(vec.times(rn))};
}
...
item.addRandomSegment = function (src,dst) {
  let srcP;
  let srcFraction;
  let dstFraction;
  if (src) {
    ...
    if (srcIsCircle) {
      srcP = this.genRandomPointInCircle(src);
    } else {
      let pos = this.genRandomPointOnSegment(src);
      srcP = pos.value;
      srcFraction = pos.fraction;
    }
  ...
  if (dst) {
    ...
    if (dstIsCircle) {
      dstP = this.genRandomPointInCircle(dst);
    } else {
      let pos = this.genRandomPointOnSegment(dst); 
      dstFraction = pos.fraction;
      dstP = pos.value;
    }
   ...
   // dstP = (dstIsCircle)?this.genRandomPointInCircle(dst):this.genRandomPointOnSegment(dst); 
    ...
    rsg.srcFraction = srcFraction;
    rsg.dstFraction = dstFraction;
...
//debugger;
 ...
 // line.stroke  = genRandomColor();
  ...
  if (seg2line) {
    ...
    let {srcFraction,dstFraction} = lsg;
    if (typeof srcFraction === 'number') {
      debugger;
      let rdiv = this.rDivisions;
      let cellSrc = Math.floor(rdiv * srcFraction);
      let cellDst = Math.floor(rdiv * dstFraction);
      let lv = this.randomizer.valueAt(this.grayLevel,cellSrc,cellDst);
      let rgb =  `rgb(${Math.floor(lv)},${Math.floor(lv)},${Math.floor(lv)})`;
      console.log('rgb',rgb);
      line.stroke = rgb;
...
//  let originatingShapes = [geom.Circle.mk(Point.mk(-100,-150),5),geom.Circle.mk(Point.mk(0,-150),5),geom.Circle.mk(Point.mk(100,-150),5)];
 // let originatingShapes = [geom.Circle.mk(Point.mk(-100,-200),45),geom.Circle.mk(Point.mk(100,-200),45)];
  ...
  if (shapePairs) {
    ...
    for (let i = 0;i < ln;i++) {
      ...
      for (let j=0;j<nlnp;j++) {
        this.addRandomSegment(cp[0],cp[1]);
      }
    }
    this.addLines();
    return;
  ...
  while (true) {
    ...
    let rsg = this.addRandomSegment(noc?ocs[which-1]:null)
    if (rsg) {
      if (ocs) {
