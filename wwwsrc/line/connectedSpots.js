...
item.initializePrototype = function () {
  ...
  spotP.fill = 'black';
  spotP.dimension = this.spotDimension;
}
...
item.update = function () {
  ...
  let line = this.line;
  if (!line) {
    line = this.set('line',svg.Element.mk('<line/>'));
    line.neverselectable = true;
  }
  line.show();
  line.stroke = this.lineStroke;
  line['stroke-width'] = this.lineWidth;
  line.setDomAttribute('x1',end0.x);
  line.setDomAttribute('y1',end0.y);
  line.setDomAttribute('x2',end1.x);
  line.setDomAttribute('y2',end1.y);
  line.draw();
  ...
  for (let i=0;i<numSpots;i++) {
    let nm = 's'+i;
    let spot = this[nm];
    spot.fill = this.spotFill;
    spot.dimension = this.spotDimension; 
    spot.show();
    spot.update();
...
ui.hide(item,['end0','end1','actualNumSpots','numSpots','line','shownSpots','stroke','stroke-width']);
