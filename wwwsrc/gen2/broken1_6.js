...
this.lineP['stroke-width'] = this.whiteOnBlack?0.2:3;;// .4;  
...
rs.initialize = function () {
	let {width,height,numRows,numCols,lineColor} = this;
	let hwd = width/2;
	let hht = height/2;
	let deltaX = width/numCols;
	let deltaY = height/numRows;
  core.root.backgroundColor = this.whiteOnBlack?'black':'cyan';
 // core.root.backgroundColor ='black';
  ...
  for (let i=0;i<numRows;i++) {
	  ...
	  //this.mkRandomlySegmentedLine(seg,5,20);
		let color = (0 && (Math.abs(i - hr) < numRows/20))?'red':lineColor;
	...
	for (let j=0;j<numCols;j++) {
	  ...
	  //this.mkRandomlySegmentedLine(seg,1,20);
	  //this.mkRandomlySegmentedLine(seg,5,20);
			let color = (0 && (Math.abs(j - hc) < numRows/20))?'red':lineColor;
    //this.mkRandomlySegmentedLine(seg,30,40,color);
    this.mkRandomlySegmentedLine(seg,30,40,color);
	}
