...
rs.setName('drop0_1_19');
let ht = 300;
let wd = 1.5 * ht;
//wd = ht; //for instagram
let topParams = {width:wd,height:ht,numSeedRows:2,numSeedCols:3,maxDrops:1000,maxTries:10,lineLength:5,backgroundColor:'rgb(2,2,2)',backgroundPadding:10,minSeparation:0,rectangleDim:0.8,gridPadding:60,fromEnds:1,sepNext:0.01,onlyFromSeeds:1,extendWhich:'first',numSeeds:60,splitChance:0,splitAmount:0.2 * Math.PI,endLoops:30000,seedDirections:[0.5*Math.PI],directionChange:0.02*Math.PI,randomDirectionChange:0.08*Math.PI}
topParams = {width:wd,height:ht,numSeedRows:0,numSeedCols:0,maxDrops:1000,maxTries:10,lineLength:5,backgroundColor:'rgb(2,2,2)',backgroundPadding:10,minSeparation:0,rectangleDim:0.2,gridPadding:60,fromEnds:1,sepNext:0.01,onlyFromSeeds:1,extendWhich:'first',numSeeds:60,splitChance:.10,splitAmount:0.005 * Math.PI,endLoops:3000,seedDirections:[0*Math.PI],directionChange:0.0*Math.PI,randomDirectionChange:0.051*Math.PI,lineExt:0}
...
rs.genSeeds = function () {
	...
	let LLS = this.genOneSegment(LL,-0.5*Math.PI,'cyan');
	segs.push(LLS[0][0]);
	lines.push(LLS[1][0]);
	let UL = Point.mk(dc - hw,dc-hh);
	let ULS = this.genOneSegment(UL,0,'cyan');
	segs.push(ULS[0][0]);
	let LR = Point.mk(hw-dc,hh-dc);
	let LRS = this.genOneSegment(LR,Math.PI,'cyan');
	segs.push(LRS[0][0]);
...
rs.genGridSegments = function (cell,p) {
  let {deltaX,deltaY,rectangleDim} = this; 
	...
	let  SL = this.genOneSegment(p.plus(Point.mk(0,-0.4*deltaY)),0.5*Math.PI,'cyan');
	let seedSeg = SL[0][0];
	segs.push(seedSeg);
	let end = seedSeg.end;
	end.seed = end;
