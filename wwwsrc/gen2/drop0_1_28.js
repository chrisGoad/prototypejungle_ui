...
topParams = {width:ht,height:ht,numRows:20,numCols:30,maxDrops:30000,maxTries:10,endLoopss:1000,lineLength:10,backgroundColor:'rgb(2,2,2)',backgroundPadding:0.1*ht,separation:0,randomDirectionChange:0.3*Math.PI,fromEnds:1,sepNext:.1,lineExt:.2,onlyFromSeeds:1,extendWhich:'random',numSeeds:10,splitChance:0.5,splitAmount:0.08 * Math.PI,directionChange:0.25 * Math.PI}
...
//	let rseed = this.segsToSeed(rsgs);
  this.ringRadius = 0.2 * 0.5 * width;
	let excl = geom.Circle.mk(Point.mk(-0.2*width,0),this.ringRadius - 1);
	let excr = geom.Circle.mk(Point.mk(0.2*width,0),this.ringRadius - 1);
	this.exclusionZones = [excl,excr];
