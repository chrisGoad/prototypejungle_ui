...
function (constructor,addMethods) {
  ...
  //let rs = constructor();
	rs.saveImage = true;
	rs.loadFromPath = 1;
	rs.setName('spatter0_8');
	grid1.setName('spatter0_8','spatter_3');
	grid2.setName('spatter0_8','spatter_3');
	grid1.initProtos();
	grid2.initProtos();
...
rs.step = function () {
	debugger;
	let {grid1,grid2} = this;
	let {timeStep}  = this;
	grid1.timeStep = grid2.timeStep = timeStep;
//	this.grid1.stepShapeRandomizer('direction');
	//this.grid2.stepShapeRandomizer('direction');
	grid1.updateSpatter();
	grid2.updateSpatter();
