...
rs.inPattern = function (x,y,kind) {
	let {numRows,numCols} = this;
  let hr = 0.5 * numRows;
	let dir;
	if (kind === 'ul') {
  ...
  rs.computeDir = function (x,y) {
		let downRight =  0.75* Math.PI;
		let downLeft =  0.25* Math.PI;
		debugger;
	  let {numRows,numCols} = this;
		let hr = 0.5 * numRows;
		let dir;
	  if (this.inPattern(x,y,'ul')) {
			dir = downLeft;//0.75* Math.PI;
			return [dir];
		} 
	  if (this.inPattern(x,y,'lr')) {
			dir = downLeft;
			return [dir];
		}  
		 if (this.inPattern(x,y,'ll')) {
			dir = downRight;
			return [dir];
		}  
		 if (this.inPattern(x,y,'ur')) {
			dir = downRight;
			return [dir];
		}  
/*		if ((x+y) === 3*hr) {
