...
rs.finishProtos = function () {
	...
	//this.lineP.stroke = 'rgb(100,100,100)';
...
rs.initialSegments = function () {
  ...
  let segs1 = this.rectangleSegments(width,height);
	let fc = 0.6;
  let segs2 = this.rectangleSegments(fc*width,fc*height);
	let fc2 = 0.45;
  let segs3 = this.rectangleSegments(fc2*width,fc2*height);
	let segs = segs1.concat(segs2,segs3);
 /* let hw = 0.5*width;
  let hh = 0.5*width;
...
rs.segParams = function () {
 ...
 // let length = 2 + Math.floor(r*np)*4;
  let length = 6 + Math.floor(r*np)*4;
