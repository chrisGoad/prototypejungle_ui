...
rs.defaultPositionFunction = function (i,j) {
  ...
  return Point.mk(botx + deltaX*i,boty + deltaY*j);
	...
	rs.adjustCell = function (x,y,delta,kind,timeRange,innerTime) {
		let {numRows,shapes,lineLength,mainRange} = this;
		let idx = x*numRows + y;
		let line0 = shapes[idx];
		let fr = y/numRows;
		let tfr = innerTime/mainRange;
		if (y === 5) {
		   debugger;
		}
		let toColor1,toColor2;
		if (timeRange < 3) {
			toColor1 = 'white';
			toColor2 = 'black';
		} else {
		...
		line0.stroke = (tfr >= fr)?toColor2:toColor1;
		
	}
	
	rs.adjustCells = function (delta,kind,timeRange,innerTime) {
	//	debugger;
		let {numRows,numCols} = this;
		for (let i=0;i<numCols;i++) {
			for (let j=0;j<numRows;j++) {
				if ((kind===1) && this.inPattern1(i,j)) {
				  this.adjustCell(i,j,delta,kind,timeRange,innerTime);
...
rs.step = function ()   {
  ...
  let range = whatTimeRange();
	...
	if (range === 1 ) {
		this.adjustCells(sp*innerSq,1,range,inner);
		return;
	} 
	if (range === 2) {
		//debugger;
		this.adjustCells(sp*innerSq,2,range,inner);
		return;
	} 
	if (range === 3) {
		//debugger;
		this.adjustCells(-sp*untilSq,2,range,inner);
		return;
	} 
	if (range === 4) {
		debugger;
		this.adjustCells(-sp*untilSq,1,range,inner);
		return;
	} 
	if (0 && (range === 5)) {
		debugger;
