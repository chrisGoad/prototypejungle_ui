...
item.addShortenedLine = function (p0,p1,proto) {
...
item.addCellBoundaries = function () { 
...
item.addShapes = function () { 
  ...
  let {numRows,numCols,shapeP1,shapeP2,replacementShapeP} = this;
  let genFun = this.shapeGenerationFunction;
  ...
  let n = 0;
  let ls = this.lineShapeLength;
  let p0 = Point.mk(-0.5*ls,0);
  let p1 = Point.mk(0.5*ls,0);
  for (let j = 0;j < numCols; j++) {
    /*if (this.fadeIn) {
      this.boundaryLineFraction = j/numCols;
    }*/
    for (let i = 0;i <  numRows; i++) {
      ...
      let shp;
    //  let repFun = this.replacementFunction;
      if (genFun) {
        shp = genFun(this,shapes,idx,i,j);
      //} else if (repFun && replacementShapeP && repFun(this,i,j)) { 
      //  shp = replacementShapeP.instantiate();
      } else if (cnt  && (Math.random() < this.chanceAshapeIsVisible)) {
        shp = shapeP1.instantiate();
      }

      /*  if (ls) {
           shp = this.addShortenedLine(p0,p1,shapeP1);
        } else if (shapeP2) {
        //let shp = (Math.random() > 0.5)?this.rectangleP.instantiate():this.circleP.instantiate();
    
          shp = (Math.random() < this.chanceShape2IsVisible)?shapeP2.instantiate():shapeP1.instantiate();
        } else {
          shp = shapeP1.instantiate();
        }
      }*/
...
item.randomizeLineColors = function (which) {
  let {lines,rlines,colorStart} = this;
...
item.addToRegion = function (region,gidx,np) {
  ...
  this.addLine(this.rlines,opnt,npnt);
}
...
item.initializeGrid = function () {
 ...
 // this.initializeProtos();
  ...
  if (this.includeLetters) {
    ...
    //let windows = this.genAwindows(3,3);
