...
item.addBrokenLine = function (lineP,lines,start,dir,ln,maxThere,minThere,maxMissing,minMissing) {
  //let {lines} = this;
  let cLen = 0;
  let cp = start;
  ...
  while (cLen < ln) {
    ...
    if (addingThere) {
      ...
      if (this.shades) {
         let shd = this.pos2shade(cp.plus(np).times(0.5));
         newLine.stroke = shd;
      }         
      newLine.update();
      newLine.show();
...
/* 
item.addBrokenLine = function (start,dir,ln,maxThere,minThere=0,maxMissing,minMissing=0) {
