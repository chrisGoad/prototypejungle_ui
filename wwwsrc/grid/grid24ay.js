...
rs.addElement = function (shape,cell,iidx,rvs,ipos) {
	...
	if (rectElement) {
    if (dim < 0.5) {
			if (exchangeColor && !sideCell) {
				let colorv = Math.floor(exchangeColor*255);
			  fill = toRgb(colorv);
			} else {
			  fill = aFill;
			}
		  stroke = aStroke;
			...
			if (innerColor && !sideCell) {
				 fill = toRgb(innerColor);
			} else if (exchangeColor&& !sideCell) {
				let colorv = Math.floor((1-exchangeColor)*255);
			  fill = toRgb(colorv);
			} else {
				fill = bFill;
			}
...
rs.initialize = function () {
	...
	if (innerColorParams) {
	  innerColorParams . numRows = numRows*innerRows;
	  innerColorParams . numCols = numRows*innerCols;
	  this.setupShapeRandomizer('innerColor',innerColorParams );
	 //this.setupShapeRandomizer('color', {step:30,min:100,max:250,numRows*innerRows,numCols*innerCols});
	}
	if (exchangeColorParams) {
	 
	  this.setupShapeRandomizer('exchangeColor',exchangeColorParams );
	 //this.setupShapeRandomizer('color', {step:30,min:100,max:250,numRows*innerRows,numCols*innerCols});
	}
  this.rValues = [];
	this.aPointValues = [];
