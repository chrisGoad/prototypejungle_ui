...
return function () {
  ...
  rs.shapeGeneratorrr = function (shapes,rvs) {
...
rs.boundaryLineGenerator = function (lines,end0,eend1,rvs,cell,orientation) {
	debugger;
	let {x,y} = cell;
	if ((Math.random() > 0.7) && (x%2 === 0) && (y%2===0) && (orientation === 'vertical')) {
		let line = this.boundaryLineP.instantiate();
		lines.push(line);
		let vec = Math.random() > 0.5?Point.mk(2*this.deltaX,0):Point.mk(0,2*this.deltaY);
