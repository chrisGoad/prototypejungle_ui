...
item.initializeOuterProtos	 = function () {
  ...
  core.assignPrototypes(outer,'lineP',linePP);
  outer.lineP.stroke = 'black';
  let vec = Point.mk((outer.width/outer.numRows)*0.3,0);
  outer.lineP.setEnds(vec.minus(),vec);
  outer.lineP['stroke-width'] = .8; 
  core.assignPrototypes(outer,'circleP',circlePP);
  outer.circleP.fill = 'red';
  outer.circleP.dimension = (outer.width/outer.numRows)*0.2;
  outer.circleP['stroke-width'] = 0; 	
...
item.initialize = function () {
 ...
 /* grid.circleCells.minx = 6;
  ...
  outer.bendCircleRadius = 180* (31/22);
  ...
  outer.shapeGenerationFunction = function (grid,shapes,idx,i,j) {
    let inAbox =  grid.inBox(circleBox1,i,j,true) || grid.inBox(circleBox2,i,j,true);
    let rs = (inAbox)?grid.circleP.instantiate():grid.lineP.instantiate();
    if (!inAbox && Math.random()<0.5) {
      return;
    }
    shapes.set(idx,rs);
    rs.update();
