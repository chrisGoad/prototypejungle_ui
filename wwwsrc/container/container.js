...
core.require('/text/combo2.js',function (comboP) {
...
item.containerUpdate =  function () {
  ...
  if (!this.border) {
    ...
    if (core.hasRole(this.border,'vertex')) {
      this.role = 'vertex';
    }
  }
  ...
  if (this.contents || this.text || this.image) {
    if (!this.contents) {
      this.set('contents',comboP.instantiate()).show();
...
item.update = item.containerUpdate;
...
item.installBorder = function (borderPP) {
   core.assignPrototype(this,'borderP',borderPP);
}
/*
item.installMethods = function (container,borderPP) {// contentsPP
  ...
  container.initializePrototype = function () {
    core.assignPrototypes(this,'borderP',borderPP,'contentsP',thisHere);
  }
}
*/
...
ui.hide(item,['outline','singleLine','text','textarea','includeOutline','firstUpdate','plainText',
              'lastDim','lastHeight','lastWidth','borderProperties','containerPropertiesHidden','containerPropertiesShown']);
