...
core.require('/line/line.js',function () {
...
item.dimension = 30;
item.dimensionRatio = 0.4;
item.numCircles = 3;
item.numSpokes= 0;
item.lineWidthFactor = .5;
item.jiggle = .2;
...
item.update =  function () {
  let {circlesBuilt,numCircles,numSpokes,elementP1,elementP2} = this;
  let isDual = !!(this.elementP2);
  ...
  if (circlesBuilt === numCircles) {
   ...
   // let dimRatio = (this.dimension - this.innerDimension)/(this.dimension);
    ...
    for (let i=0;i<circlesBuilt;i++) {
      let strk = interpolateColor(this.outerStroke,this.innerStroke,i/(circlesBuilt-1));
      colors.push(strk);
      dimensions.push( dim + (innerDimension - dim)*(i/(circlesBuilt-1)));
    ...
    let initialAngleDelta = 360/circlesBuilt;
    let initialAngle = 0;
    for (let i=0;i<circlesBuilt;i++) {
      let c = this['c'+i];
      if (!c) {
  ...
  if (circlesBuilt > numCircles) {
    for (let i=numCircles; i<circlesBuilt;i++) {
      this['c'+i].remove();
      //this['s'+i].remove();
  ...
  for (let i=circlesBuilt; i<numCircles;i++) {
   // let c = this.set('c'+i,svg.Element.mk('<circle/>'));
    ...
    if (Math.random() < this.ringVisProbability) {
