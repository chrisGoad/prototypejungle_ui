...
item.vertical = false; //  the first leg is vertical
item.stroke = "black";
...
item.initialize = function () {
  ...
  let numBends = this.numBends;
  let bendPoints = this.bendPoints;
  if (bendPoints === undefined) {
    bendPoints = core.ArrayNode.mk(numBends);
    this.set('bendPoints',bendPoints);
    let increment = vertical?Point.mk(0,50):Point.mk(50,0);
...
item.update = function () {
  ...
  let oddNumBends = (numBends % 2) === 1;
  this.__connectionType = (vertical === oddNumBends)?'EastWest':'UpDown';
