...
function (rs) {
 ...
 rs.setName('grid0_20_13');
 let threeD = 0;
 let bkdim = threeD?1200:40;
let gdim = 32;
let topParams = {width:40,height:40,numRows:gdim,numCols:gdim,pointJiggle:0,fcTriX:1,fcTriY:1,fcLineX:.4,fcLineY:.7,fcGonX:.5,fcGonY:.5,fcCircle:0.7,numTimeSteps:numTimeSteps,frameSpeed:0.02,timeSpeed:0.05,interpolateFromStep:undefined,interpolateSteps:0,
...
rs.chooseWhich = function (rvs, cell) {
  ...
  if (((timeStep < 100)&&(((x/nc < fr)&&(y/nr < fr))||(((nc-x)/nc < fr)&&((nr-y)/nr < fr)))) ||  
      ((timeStep>=100) && (((x/nc < fr)&&((nr-y)/nr < fr))||(((nc-x)/nc < fr)&&(y/nr < fr))))){
  //if (((x/nc < fr)&&((nr-y)/nr < fr))||(((nc-x)/nc < fr)&&(y/nr < fr))){
 // if ((xsq + ysq)%2 === 0) {
