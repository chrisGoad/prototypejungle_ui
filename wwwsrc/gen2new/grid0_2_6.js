...
let p3 = pr.instantiate();
let p4 = pr.instantiate();

p1.setName(null,'grid0_2_0');
p2.setName(null,'grid0_2_0');
p3.setName(null,'grid0_2_0');
p4.setName(null,'grid0_2_0');
...
p3.patternDir = p1.patternDir;
p3.patternOp = function (line) {
	line.stroke = 'red';
	//line['stroke-width'] = 2;
}


p4.patternOp = function (line) {
	line.stroke = 'red';
}

rs.setName('grid0_2_6');

let pdim = 300;
let twid = 2.2*pdim;
let bpad = 50;
let partParams = {loadFromPath:1,saveJson:0,numRows:20,width:pdim,height:pdim,numCols:20};
let topParams = {width:twid,height:twid,backgroundColor:'rgb(2,2,2)',backgroundPadding:0.1*twid};
Object.assign(p1,partParams);
Object.assign(p2,partParams);
Object.assign(p3,partParams);
Object.assign(p4,partParams);
...
rs.initialize = function () {
    ...
    p2.initialize(() => {
			p3.initialize(() => {
				p4.initialize(() => {
					let mv = 0.6 * p1.width;
					p1.moveto(Point.mk(mv,-mv,0));
					p2.moveto(Point.mk(-mv,-mv,0));
					p3.moveto(Point.mk(mv,mv,0));
					p4.moveto(Point.mk(-mv,mv,0));
