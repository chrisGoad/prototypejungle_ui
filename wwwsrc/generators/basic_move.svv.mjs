...
rs.initialize = function () {
  ...
  let hdim = dim/2;
	this.addBackStripe();
	this.addShapes('h2',4,dim);
	this.addShapes('v2',4,dim);
	this.addShapes('h3',5,dim);
	this.addShapes('v3',5,dim);
  ...
  this.h2[3].moveto(right); 
  this.v2[3].moveto(bottom); 
...
rs.computePositions = function (i) {
  ...
  rs.push(fc*iv-hdim);
  if (i===3) {
    console.log(rs[0],rs[1],rs[2],rs[3]);
  }
  return rs;
}
...
rs.moveRect = function (nm,psa,n) {
  ...
  let vrwh = pshigh - pslownn;
  let vrp = (pshigh + pslownn)/2;
  console.log('pslow',pslow,'pshigh',pshigh,'vrp',vrp);
...
rs.step = function () {
 ...
 /* let all = ps2.concat(ps2,ps3);
  ...
  //ps3.sort();  
  this.moveRect('vr',ps3,0);
  this.moveRect('vr',ps3,1);
  this.moveRect('vr',ps3,2);
  //this.moveRect('vr',ps3,3);
  
  //s2[3].moveto(Point.mk(ps3[3],0));
  //console.log('x0',x0,'x1',x1);
  h2[1].moveto(Point.mk(ps2[0],0));
  v2[1].moveto(Point.mk(0,ps2[0]));
