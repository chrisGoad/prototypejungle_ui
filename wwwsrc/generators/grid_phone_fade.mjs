...
let nc = 64;
let nr = Math.floor((4096/1890)*nc);
//nr = 48;
let wd = 200;
let ht = (4096/1890)*200;
let topParams;
if (radial) {
  topParams = {numRows:0.5*nr,numCols:nr,width:wd,height:ht,outerRadius:wd,innerRadius:0.2*wd,angleMin:-180,angleMax:180,center: Point.mk(0,0),rotation:30,backStripeColor:'rgb(2,2,2)',pointJiggle:4,backStripePadding:0.15*wd};
} else {
  topParams = {numRows:nr,numCols:nc,width:wd,height:ht,backStripeColor:'rgb(2,2,2)',pointJiggle:4,backStripePadding:0.15*wd,backStripeVisible:0,numTimeSteps:10,signIt:0};
}
...
rs.boundaryLineGenerator= function (end0,end1,rvs,cell) {
	let {blineP,numRows,numCols,showMissing,lines,updating,lineIndex} =this;
  let {x,y} = cell;
  let th = 1- (y)/(numRows);
  let rr = Math.random();
